...
const NON_SEND_PATTERNS = [
  ...
  /帮我写/,
  /帮我.*写/,
  /(?:写|生成|创作|撰写|起草|制作).*(?:文案|文章|脚本|口播|内容|报告|方案|word|docx|文档|ppt|pdf|文件)/i,
  /(?:整理成|整理为|转成|转为).*(?:word|docx|文档|ppt|pdf|文件)/i,
  /(?:保存|存放|存到|存).*(?:桌面|desktop|下载|downloads|文档|documents|文件夹|目录)/i,
  /改一下/,
  /润色/,
];
